# portworx AutoShift Policy

## Overview
This policy installs the portworx operator using AutoShift patterns.
...
helm template policies/portworx/
```
...
hubClusterSets:
  hub:
    labels:
      portworx: 'true'
      portworx-subscription-name: 'portworx'
      portworx-channel: 'stable'
      portworx-source: 'redhat-operators'
      portworx-source-namespace: 'openshift-marketplace'
      # portworx-version: 'portworx.v1.x.x'  # Optional: pin to specific CSV version
...
clusters:
  my-cluster:
    labels:
      portworx: 'true'
      portworx-channel: 'fast'  # Override channel for this cluster
```
...
- **Version Pinning**: Add `portworx-version` label to pin to a specific CSV version
...
```yaml
portworx-version: 'portworx.v1.x.x'
```

Find available CSV versions:
```bash
# List available versions for this operator
oc get packagemanifests portworx -o jsonpath='{.status.channels[*].currentCSV}'
```
...
#### Common Patterns:
- `policy-portworx-config.yaml` - Main configuration
- `policy-portworx-<feature>.yaml` - Feature-specific configs

#### Template Structure:
```yaml
{{- $policyName := "policy-portworx-config" }}
{{- $placementName := "placement-policy-portworx-config" }}
...
spec:
  disabled: false
  dependencies:
    - name: policy-portworx-operator-install
      namespace: {{ .Values.policy_namespace }}
      apiVersion: policy.open-cluster-management.io/v1
          ...
          object-templates:
            - complianceType: musthave
              objectDefinition:
                ...
                metadata:
                  name: portworx-config
                  namespace: {{ .Values.portworx.namespace }}
                spec:
                  # Your operator-specific configuration
                  # Use dynamic labels when needed:
                  # setting: '{{ "{{hub" }} index .ManagedClusterLabels "autoshift.io/portworx-setting" | default "default-value" {{ "hub}}" }}'
...
# portworx-setting<string>: Configuration option (default: 'value')
# portworx-feature-enabled<bool>: Enable optional feature (default: 'false')
# portworx-provider<string>: Provider-specific config (default: 'generic')
...
3. Check policy status: `oc describe policy policy-portworx-operator-install`

### Operator Installation Issues
1. Check subscription: `oc get subscription -n portworx`
2. Check install plan: `oc get installplan -n portworx`
3. Verify operator source exists: `oc get catalogsource -n openshift-marketplace`

### Template Rendering Issues
1. Test locally: `helm template policies/portworx/`
2. Check hub escaping: Look for `{{ "{{hub" }} ... {{ "hub}}" }}` patterns
3. Validate YAML: `helm lint policies/portworx/`

## Resources
- [Operator Documentation](https://operatorhub.io/operator/portworx) - Find your operator details
