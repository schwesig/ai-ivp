...
## Overview
This policy installs the keycloak-operator operator using AutoShift patterns.
...
hubClusterSets:
  hub:
    labels:
      keycloak: 'true'
      keycloak-subscription-name: 'keycloak-operator'
      keycloak-channel: 'stable'
      keycloak-source: 'redhat-operators'
      keycloak-source-namespace: 'openshift-marketplace'
      # keycloak-version: 'keycloak-operator.v1.x.x'  # Optional: pin to specific CSV version
...
```yaml
keycloak-version: 'keycloak-operator.v1.x.x'
```

Find available CSV versions:
```bash
# List available versions for this operator
oc get packagemanifests keycloak-operator -o jsonpath='{.status.channels[*].currentCSV}'
```
...
oc get pods -n keycloak-manager
...
1. Check subscription: `oc get subscription -n keycloak-manager`
2. Check install plan: `oc get installplan -n keycloak-manager`
...
## Resources
- [Operator Documentation](https://operatorhub.io/operator/keycloak-operator) - Find your operator details
